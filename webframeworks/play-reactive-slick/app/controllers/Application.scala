...
import com.fasterxml.jackson.module.scala.{DefaultScalaModule, TupleModule, OptionModule}
import play.api.http.{Writeable, ContentTypes}
...
//  }
  ...
  implicit def writeableOfAny(implicit codec: Codec): Writeable[Any] = Writeable(serializeToJson, Some(ContentTypes.JSON))


  def serializeToJson(data: Any): Array[Byte] = {
    mapper.writeValueAsString(data).getBytes
  }

  def index = Action.async { implicit request =>
    dao.findById(1).map { employee =>
      Ok(employee)
    }.recover {
